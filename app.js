(function() {
  ...
  function renderExpenses() {
    ...
    for (var a = 0; a < ks.length; a++) {
      ...
      h += '<div class="day"><div class="day-hdr"><span class="d">' + formatDateHeader(ks[a]) +
      ...
      for (var x = 0; x < it.length; x++) {
        ...
        h += '<div class="item' + ni + '" data-idx="' + idx + '" data-id="' + id +
          '" tabindex="0" role="button"><input type="checkbox" class="cb' + cb + '" data-id="' + id +
          ...
          .p + '" data-d="' + (i.d || '') + '" data-idx="' + i.idx + '">🗑</button></div>';
        idx++
      }
      h += '</div></div>'
    }
  ...
  function renderCalendar(cb) {
    ...
    var h_ = '<div class="ch"><button id="cpr">◀</button><span class="my">' + JALALI_MONTHS[calendarMonth -
      1] + ' ' + toPersianDigits(calendarYear) + '</span><button id="cnx">▶</button></div>';
    h_ += '<div class="cdh">';
